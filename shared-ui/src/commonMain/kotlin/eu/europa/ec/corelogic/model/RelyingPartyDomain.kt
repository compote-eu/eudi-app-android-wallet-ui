...
// Phase 4: the registration-certificate domain, in commonMain because the consent and success
// screens that render it are. `logoUri` is a String, not `java.net.URI` — the same reason
// `IssuerDetailsCardDataUi.issuerLogo` is (see the note there): URI is JVM-only, and every consumer
// hands it straight to an image loader as a string anyway. The Wallet Core mapping stays Android, in
// :core-logic's RegistrationCertificateExtensions.
