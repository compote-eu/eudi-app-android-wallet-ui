 ...
 * [RelyingPartyDomain.isFullyVerified] — the same verdict every other screen of the presentation
 * renders — while registration details render whenever a certificate was parsed, even one that
 * failed validation.
 */
fun RelyingPartyDomain.toRelyingPartyHeaderUi(fallbackName: UiText): RelyingPartyHeaderUi {
    ...
    return RelyingPartyHeaderUi(
        relyingParty = RelyingPartyDataUi(
            ...
            name = name.asUiTextOr(fallback = fallbackName),
            uniqueId = uniqueId,
            description = null,
        ),
