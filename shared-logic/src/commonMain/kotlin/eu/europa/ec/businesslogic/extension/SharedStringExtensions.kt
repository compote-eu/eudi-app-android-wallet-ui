...
// One helper out of :business-logic's `StringExtensions`, moved because `DocumentOfferInteractorImpl`
// needs it in commonMain — an issuer that publishes a blank name gets the wallet's default. The rest of
// that file stays on Android; it is regex, encoding and `TextUtils` work with no shared caller.
// Package unchanged.
//
// **The file name differs from the Android one on purpose.** Same package + same file name would compile
// to the same JVM facade class (`StringExtensionsKt`) twice on the app's classpath, and one would shadow
// the other — which is exactly how this first showed up: `ifEmptyOrNull` vanished at runtime with a
// NoSuchMethodError while compiling perfectly.
