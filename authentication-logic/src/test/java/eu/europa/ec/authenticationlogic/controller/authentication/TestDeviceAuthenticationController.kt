...
class TestDeviceAuthenticationController {
    ...
    /**
     * A canary, not a test of our own code: the reason [DeviceAuthenticationControllerImpl] derives
     * the authenticators from the crypto handle at all is that AndroidX refuses this combination
     * outright. If this ever stops throwing, the branch is no longer load-bearing.
     */
    @Test
    fun `AndroidX still refuses a crypto request that allows weak biometrics`() {
        ...
        val thrown = assertThrows(IllegalArgumentException::class.java) {
            prompt.authenticate(info, requireNotNull(crypto.cryptoObject))
        }

        assertTrue(thrown.message.orEmpty().contains("Class 2 (Weak)"))
    }
