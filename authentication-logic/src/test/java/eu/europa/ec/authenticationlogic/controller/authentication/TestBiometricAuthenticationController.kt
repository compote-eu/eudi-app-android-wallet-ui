...
class TestBiometricAuthenticationController {
    ...
    fun before() {
        ...
        // test cannot read.
        whenever(resourceProvider.getString(Res.string.biometric_strong_required))
            .thenReturn(STRONG_REQUIRED_MESSAGE)
        whenever(resourceProvider.getString(Res.string.biometric_no_hardware))
            .thenReturn(NO_HARDWARE_MESSAGE)
        whenever(resourceProvider.getString(Res.string.biometric_unknown_error))
    ...
    // passed, the STRONG prompt then errored, and the old retry re-opened it forever)
    @Test
