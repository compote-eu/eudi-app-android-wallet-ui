...
# TC-02 - orchestrates the Android (Maestro) and iOS (Appium) halves of a
# BLE proximity presentation with a custom PID attribute subset.
#
...
# (tc-02-ble-custom-attributes.yaml blocks on extendedWaitUntil while the
...
#     ANDROID_UDID=<serial> EUDI_WALLET_PIN=<pin> ./tc-02-ble-custom-attributes.sh
set -euo pipefail
...
LOGCAT_FILE="$(mktemp -t tc02-android-logcat)"
...
echo "--- Step 2: start the Android flow in the background (create custom request, open scanner, wait for result) ---"
MAESTRO_TESTS_DIR="$HOME/.maestro/tests"
MAESTRO_START_MARKER="$(mktemp -t tc02-maestro-marker)"
maestro --udid "$ANDROID_UDID" test "$SCRIPT_DIR/tc-02-ble-custom-attributes.yaml" &
ANDROID_PID=$!
...
set +e
python3 "$SCRIPT_DIR/tc-02-ios-share.py" "$IOS_UDID" "$APPLE_TEAM_ID" "$IOS_BUNDLE_ID"
IOS_EXIT=$?
set -e
...
set +e
python3 "$SCRIPT_DIR/tc-02-verify-attributes.py" "$LOGCAT_FILE"
VERIFY_EXIT=$?
set -e

if [ "$IOS_EXIT" -ne 0 ] || [ "$ANDROID_EXIT" -ne 0 ] || [ "$VERIFY_EXIT" -ne 0 ]; then
  echo "TC-02: FAIL (ios_exit=$IOS_EXIT android_exit=$ANDROID_EXIT verify_exit=$VERIFY_EXIT)."
