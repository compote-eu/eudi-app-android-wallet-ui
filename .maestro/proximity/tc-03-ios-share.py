...
"""TC-03 - iOS half: trigger BLE proximity share (Home -> Authenticate ->
...
Identical to TC-02's iOS half - confirmed live (2026-08-27) that
requesting the full PID (vs. TC-02's custom subset) changes nothing on
the iOS side: the consent/share screen's button is gated only by
`allowShare` + relying-party trust, not by which/how many fields were
requested, and no different screen appears. Copied rather than shared
via a common module - this project's existing .maestro/*/tc-*.py
scripts are each self-contained, no cross-script imports - but keep
both files' fixes in sync if either needs another one.
...
Usage: tc-03-ios-share.py <physical-iphone-udid> <apple-team-id> <bundle-id>
"""
import json
import os
import sys
...
def main():
    if len(sys.argv) != 4:
        print(
            "usage: tc-03-ios-share.py <udid> <apple-team-id> <bundle-id>",
            file=sys.stderr,
        )
        sys.exit(2)
